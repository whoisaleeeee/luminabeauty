...
public class EnvioBLImpl implements EnvioBL {
    ...
    public Envio marcarEnvioDevuelto(int idEnvio) throws Exception {
        try {
            ...
            if ("DEVUELTO".equalsIgnoreCase(envio.getEstado())) {
                throw new Exception("El envío ya se encuentra devuelto.");
            }

            if (envio.getPedido() == null ||
                    envio.getPedido().getId_pedido() <= 0) {
                throw new Exception("El envío no tiene un pedido válido asociado.");
            }

            Pedido pedido = pedidoDAO.buscarPorId(
                    envio.getPedido().getId_pedido()
            );
    ...
    private void revertirPuntosPorDevolucion(Pedido pedido) throws Exception {
        ...
        // El pedido puede no haber generado puntos. En ese caso,
        // se actualiza como devuelto sin registrar un ajuste.
        ...
        if (saldoAnterior < puntosARevertir) {
            throw new Exception(
                    "No se puede completar la devolución porque el cliente ya utilizó los puntos obtenidos en esta compra."
            );
        }
