...
public class ProfileAppService
{
    ...
    public async Task<ProfileActionResult> InitializeAsync(Cliente? sessionClient)
    {
        ...
        try
        {
            ...
            State.CurrentClient = await CargarClienteAsync(sessionClient.Id);
            ...
            State.Addresses = await CargarDireccionesAsync(State.CurrentClient.Id);
    ...
    public async Task<ProfileActionResult> GuardarPerfilAsync()
    {
        ...
        try
        {
            ...
            State.CurrentClient.Nombre = State.ProfileFirstName.Trim();
            State.CurrentClient.Apellido = State.ProfileLastName.Trim();
            State.CurrentClient.Correo = State.ProfileEmail.Trim();
            State.CurrentClient.Telefono = State.ProfilePhone.Trim();

            var actualizado = await ActualizarPerfilAsync(State.CurrentClient);
    ...
    public async Task<ProfileActionResult> MarcarPrincipalAsync(Direccion address)
    {
        if (State.CurrentClient is null)
        {
            return new ProfileActionResult(
    ...
    public async Task<ProfileActionResult> AbrirDetallePedidoAsync(Pedido order)
    {
        try
        {
            ...
            var pedidoCompleto = await _pedidoAppService.BuscarPedidoAsync(order.IdPedido);
    ...
    {
        return State.CurrentClient?.DireccionPrincipal?.IdDireccion == address.IdDireccion;
    }
    ...
    private async Task RecargarDireccionesAsync()
    {
        ...
        try
        {
            State.CurrentClient = await CargarClienteAsync(State.CurrentClient.Id);
    ...
    {
        var cliente = await _clienteAppService.ObtenerClienteAsync(idCliente);
    ...
    {
        var actualizado = await _clienteAppService.ActualizarClienteAsync(cliente);
    ...
    private Task<Direccion?> GuardarDireccionInternaAsync(
        Direccion direccion,
        Cliente cliente)
    {
        ...
        return _clienteAppService.GuardarDireccionAsync(direccion);
    }
    ...
    public string ObtenerInicialesCliente()
    {
        ...
        return string.IsNullOrWhiteSpace(iniciales) ? "LB" : iniciales;
    }
}